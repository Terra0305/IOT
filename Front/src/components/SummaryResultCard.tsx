...
interface ActivityContent {
    ...
    headline: string;
    advice: string;           
    suggestion?: string;      // <-- 이 줄이 꼭 있어야 합니다! (중간 제안 멘트)
    recommendation: string;   
    recommendationSub?: string; // <-- 이 줄도 꼭 있어야 합니다! (추천 제목 두 번째 줄)
    fullList: string[];
    list?: string[];
}
...
// --- 2. 날씨 상태별 콘텐츠 맵 (이모지를 텍스트로 변경하여 스타일 통일) ---
// --- 2. 날씨 상태별 콘텐츠 맵 (멘트 추가 및 수정) ---
const ACTIVITY_MAP: Record<StatusKey, ActivityContent> = {
    ...
    neutral: {
        ...
        // neutral/bad는 두 번째 줄이 없으면 안 나옵니다.
        fullList: [
...
export default function SummaryResultCard({ weatherData = DUMMY_WEATHER_DATA }: SummaryResultCardProps) {
    const content = getSummaryContent(weatherData); 

    return (
        <div className="summary-card-container h-full flex flex-col">
          
          {/* [간격 A] 헤더와 내용 사이: mb-10 (넓게) */}
          ...
          <div className="flex flex-col text-white">
            ...
            {/* 1. 날씨 상태 ("미세먼지 좋음...") */}
            {/* [간격 B] 상태설명 - 제안멘트 사이: mb-3 (좁게, 문단처럼 보이게) */}
            <p className="text-[39px] leading-relaxed font-medium mb-[150px] opacity-90">
              {content.advice}
            </p>

            {/* 2. [NEW] 제안 멘트 ("커피 한 잔 어떠세요?") */}
            {/* [간격 C] 제안멘트 - 추천제목 사이: mb-16 (여기를 넓혀서 구분감 줌) */}
            {content.suggestion && (
            ...
            {/* 3. 추천 제목 1줄 ("테라스 무드...") */}
            {/* [간격 D] 제목 1줄 - 2줄 사이: mb-1 (아주 가깝게 붙임) */}
            <p className="text-[37px] font-bold mb-4">
                {content.recommendation}
            </p>

            {/* 4. [NEW] 추천 제목 2줄 ("또는 야외 활동") */}
            {/* [간격 E] 제목 - 리스트 사이: mb-5 (적당히 띄움) */}
