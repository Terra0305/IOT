// src/components/WeatherIcon.tsx

import * as React from "react";

// 1. [Type Definition] SVG 경로(Path)의 속성 정의
interface IconPath {
    d: string;
    fill?: string;
    stroke?: string;
    strokeWidth?: string;
    strokeLinecap?: "butt" | "round" | "square" | "inherit";
}

// 2. [Type Definition] 아이콘 데이터 구조 정의
interface IconData {
    viewBox: string;
    paths: IconPath[];
}

// 3. [Type Definition] 가능한 날씨 상태 키(Key) 정의
// (오타 방지 및 자동 완성을 위해 필수입니다)
export type WeatherConditionKey = 
    | "sun" 
    | "cloud_sun" 
    | "cloud" 
    | "moon" 
    | "sunrise_sunset" 
    | "rain" 
    | "snow"
    // 추가적으로 필요한 키가 있다면 여기에 추가 (예: 'cloudy_night')
    | "cloudy_night"; 

// 4. [Data] ICON_MAP에 타입 적용 (Record<Key, Value>)
export const ICON_MAP: Record<WeatherConditionKey, IconData> = {
    sun: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M50 50 A20 20 0 1 0 50 50", fill: "#FFD700" }, 
            { d: "M50 10v10 M50 80v10 M10 50h10 M80 50h10", stroke: "#FFD700", strokeWidth: "5", strokeLinecap: "round" },
            { d: "M26.8 26.8l7.07 7.07 M66.13 66.13l7.07 7.07 M26.8 73.2l7.07-7.07 M66.13 33.87l7.07-7.07", stroke: "#FFD700", strokeWidth: "5", strokeLinecap: "round" }
        ]
    },
    cloud_sun: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M75 30 A18 18 0 1 0 75 30", fill: "#FFD700" }, 
            { d: "M70 65 C70 65 65 45 45 45 C25 45 20 65 20 65 C15 80 30 90 45 90 C60 90 75 80 75 65 Z", fill: "#D3D3D3" }
        ]
    },
    cloud: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M70 50 C70 50 65 30 45 30 C25 30 20 50 20 50 C15 65 30 75 45 75 C60 75 75 65 75 50 Z", fill: "#A9A9A9" }
        ]
    },
    moon: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M50 25 A25 25 0 1 0 50 75 A20 20 0 1 1 50 25 Z", fill: "#F0F8FF" }
        ]
    },
    sunrise_sunset: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M50 80 A30 30 0 0 0 20 80 H80 A30 30 0 0 0 50 80 Z", fill: "#FF8C00" },
            { d: "M10 80 H90", stroke: "#8B4513", strokeWidth: "4" }
        ]
    },
    rain: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M70 50 C70 50 65 30 45 30 C25 30 20 50 20 50 C15 65 30 75 45 75 C60 75 75 65 75 50 Z", fill: "#A9A9A9" },
            { d: "M35 80v10 M50 85v10 M65 80v10", stroke: "#4169E1", strokeWidth: "4", strokeLinecap: "round" }
        ]
    },
    snow: {
        viewBox: "0 0 100 100",
        paths: [
            { d: "M70 50 C70 50 65 30 45 30 C25 30 20 50 20 50 C15 65 30 75 45 75 C60 75 75 65 75 50 Z", fill: "#A9A9A9" },
            { d: "M35 85 L30 90 L40 90 Z M55 85 L50 90 L60 90 Z M45 80 V90 M40 85 H50 M35 75 L45 85", fill: "#FFFFFF", stroke: "#FFFFFF", strokeWidth: "1" },
            { d: "M35 85 L30 90 L40 90 Z M55 85 L50 90 L60 90 Z M35 75 L45 85", fill: "#FFFFFF", stroke: "#FFFFFF", strokeWidth: "1" },
            { d: "M30 80 H40 M35 75 V85", stroke: "#FFFFFF", strokeWidth: "2", strokeLinecap: "round" },
            { d: "M55 80 H65 M60 75 V85", stroke: "#FFFFFF", strokeWidth: "2", strokeLinecap: "round" }
        ]
    },
    // (임시) cloudy_night 키 추가 (필요시 아이콘 데이터 수정)
    cloudy_night: {
        viewBox: "0 0 100 100",
        paths: [
             { d: "M50 25 A25 25 0 1 0 50 75 A20 20 0 1 1 50 25 Z", fill: "#F0F8FF" },
             { d: "M70 65 C70 65 65 45 45 45 C25 45 20 65 20 65 C15 80 30 90 45 90 C60 90 75 80 75 65 Z", fill: "#D3D3D3" }
        ]
    }
};

// 5. [Type Definition] Component Props 정의
interface WeatherIconProps extends React.SVGProps<SVGSVGElement> {
    condition: string; // 혹은 WeatherConditionKey 로 더 좁힐 수 있음
    size?: number;
}

/**
 * 시간별/주간별 날씨 아이콘을 렌더링하는 컴포넌트
 */
const WeatherIcon: React.FC<WeatherIconProps> = ({ condition, size = 50, ...props }) => {
    
    // [Error 7053 해결]: condition을 WeatherConditionKey 타입으로 단언(Assertion)하거나
    // 안전하게 접근하기 위해 키가 존재하는지 확인합니다.
    const iconKey = (condition as WeatherConditionKey) in ICON_MAP 
        ? (condition as WeatherConditionKey) 
        : 'sun';

    const iconData = ICON_MAP[iconKey];

    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width={size}
            height={size}
            viewBox={iconData.viewBox}
            fill="none"
            {...props}
        >
            {/* [Error 7006 해결]: path와 index의 타입이 IconData 인터페이스 덕분에 자동 추론됨 */}
            {iconData.paths.map((path, index) => (
                <path
                    key={index}
                    d={path.d}
                    fill={path.fill || "none"}
                    stroke={path.stroke || "none"}
                    strokeWidth={path.strokeWidth || "none"}
                    strokeLinecap={path.strokeLinecap as any} // strokeLinecap 타입 호환성 해결
                />
            ))}
        </svg>
    );
};

export default WeatherIcon;